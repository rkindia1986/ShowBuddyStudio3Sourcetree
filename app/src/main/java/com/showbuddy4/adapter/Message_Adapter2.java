...
public class Message_Adapter2 extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    public ArrayList<QBChatDialog> arrayList = new ArrayList<>();
    public ArrayList<QBChatDialog> selected_usersList = new ArrayList<>();
    Context context;
    ...
    public Message_Adapter2(ArrayList<QBChatDialog> arrayList, ArrayList<QBChatDialog> selected_usersList, Context context) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder.getItemViewType() == VIEW_TYPE_LOADING) {
            ...
            if (holder.getItemViewType() == SENDER) {
                ((SenderMessageHolder) holder).txt_send_msg.setText(arrayList.get(position).getLastMessage());
                ((SenderMessageHolder) holder).send_time.setText(arrayList.get(position).getName());
...
//                    ((SenderMessageHolder) holder).layout_main_sender.setBackgroundColor(context.getResources().getColor(R.color.list_item_selected_state));
                ...
                ((ReceiverMessageHolder) holder).txt_recei_msg.setText(arrayList.get(position).getLastMessage());
                ((ReceiverMessageHolder) holder).recei_time.setText(arrayList.get(position).getName());
                if (!TextUtils.isEmpty(arrayList.get(position).getPhoto())) {
                    Glide.with(context)
